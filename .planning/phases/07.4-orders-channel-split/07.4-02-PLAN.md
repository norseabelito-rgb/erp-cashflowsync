...
must_haves:
  truths:
    - "Error panel shows all channels' errors regardless of active tab (date/store filters do NOT apply to error panel)"
    - "Error panel is collapsible with expand/collapse toggle"
    - "Each error shows channel badge derived from ProcessingError.orderId -> Order.source"
  ...
  key_links:
    ...
    - from: "src/components/orders/processing-errors-panel.tsx"
      to: "Order.source"
      via: "ProcessingError.orderId join to get channel badge"
      pattern: "order\\.source|errors.*source"
---
