...
must_haves:
  truths:
    - "POST /api/orders/manual creates order in Shopify first"
    - "Order saved to local DB with source='manual'"
    - "Shopify order number linked to local order"
    - "Error returns Romanian message without creating partial data"
    - "Products matched by SKU in Shopify store"
  artifacts:
...
interface CreateDraftOrderInput {
  lineItems: Array<{
    variantId?: string;  // Shopify variant ID if known
    sku?: string;        // SKU to match if variantId not known
    title: string;
    quantity: number;
...
async createDraftOrder(input: CreateDraftOrderInput): Promise<{
  ...
  // First, resolve SKUs to variant IDs if needed
  const lineItems = await Promise.all(input.lineItems.map(async (item) => {
    if (item.variantId) {
      return {
    ...
    // If no variant ID, use custom line item with title and price
    return {
      title: item.title,
      quantity: item.quantity,
      price: item.price,
...
<success_criteria>
- POST /api/orders/manual creates order in Shopify first
- Draft order converted to real order
- Local order saved with source="manual"
- Shopify order number linked correctly
- Error handling prevents partial data
- Manual orders visible in Shopify admin with tags
</success_criteria>
