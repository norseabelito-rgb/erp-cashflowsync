...
must_haves:
  truths:
    ...
    - "Store dropdown shows Shopify stores on Shopify tab, TrendyolStores on Trendyol tab"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/app/(dashboard)/orders/page.tsx"
      to: "prisma.trendyolStore.findMany"
      via: "Separate query for TrendyolStores when tab=trendyol"
      pattern: "trendyolStores|TrendyolStore"
---
...
5. Update store filter behavior with separate queries for each channel:

   a) For Shopify tab (regular Store model):
      - Use existing stores from the storesData query
      - These are Shopify stores from `prisma.store.findMany()`

   b) For Trendyol tab (TrendyolStore model):
      - Add a new React Query for TrendyolStores:
        ```typescript
        const { data: trendyolStoresData } = useQuery({
          ...
          enabled: activeTab === "trendyol",
        });
        ```
      - Transform TrendyolStores to match Store dropdown shape: `{ id, name }`
      - TrendyolStore has `id` and `name` fields directly usable

   c) For Temu tab:
      - Show "Toate magazinele" (All stores) as only option (disabled)

   d) Auto-clear logic:
      - On tab switch, check if current storeFilter exists in new channel's stores
      - If not found, reset storeFilter to "all"
      - Use: `setStoreFilter("all")` in handleTabChange if store mismatch
