...
Example 1 - ORDER_NOT_FOUND (~line 190):
```typescript
// BEFORE:
if (!order) {
  return {
    success: false,
    error: `Comanda cu ID ${orderId} nu a fost găsită`,
...
if (!order) {
  return {
    success: false,
    error: getInvoiceErrorMessage("ORDER_NOT_FOUND"),
    errorCode: "ORDER_NOT_FOUND",
  };
}
```

Example 2 - NO_COMPANY (~line 224):
```typescript
// BEFORE:
if (!company) {
  return {
    success: false,
    error: `Magazinul "${store.name}" nu are o firmă de facturare asociată.`,
...
Example 3 - NO_CREDENTIALS (~line 232):
```typescript
// BEFORE:
if (!facturisUser || !facturisPassword) {
  return {
    success: false,
    error: `Credențialele Facturis nu sunt configurate pentru firma "${company.name}"`,
...
Apply this pattern to all error returns:
- ~Line 200: ALREADY_ISSUED
- ~Line 210: TRANSFER_PENDING
- ~Line 244: NO_FACTURIS_CIF
- ~Line 252: NO_LINE_ITEMS

This ensures consistent Romanian messaging and easier future maintenance.
  </action>
  <verify>
