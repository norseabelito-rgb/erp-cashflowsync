...
must_haves:
  ...
  key_links:
    ...
    - from: "src/lib/dashboard-stats.ts"
      ...
      pattern: "prisma\\.(order|awb|invoice)\\.(count|findMany|aggregate|groupBy)"
    - from: "src/lib/dashboard-stats.ts"
      to: "DashboardCharts"
      via: "salesData array for chart"
      pattern: "salesData.*groupBy|salesData.*\\[\\]"
---
...
7. Create async function getFilteredDashboardStats(filters: DashboardFilters):
   ...
   - Run all queries with baseWhere applied:
     ...
     a. Order counts (ALL should use baseWhere):
        ...
        - pendingOrders: count with { ...baseWhere, status: "PENDING", invoiceId: null }
          IMPORTANT: Add invoiceId: null to exclude already-invoiced orders (requirement: "De Procesat" = not invoiced)
        - validatedOrders: count with { ...baseWhere, status: "VALIDATED", invoiceId: null }
          IMPORTANT: Add invoiceId: null to exclude already-invoiced orders (requirement: "De Procesat" = not invoiced)
     ...
     h. Sales data for chart (REQUIRED - filtered by date and store):
        - Query orders grouped by date with sum of totalPrice
        - Use createdAt date, apply baseWhere (storeId + date range)
        - Return array of { date: string, total: number } for Recharts
        - Example query:
          ```typescript
          ...
          // Transform to { date: 'YYYY-MM-DD', total: number }
          ```
        - If date range not specified (today default), return today's sales only
...
5. Update DashboardCharts props:
   - Pass stats.salesData to DashboardCharts (now filtered in dashboard-stats.ts)
   - Remove currentStoreId prop if still passed (filter is global now)

6. VERIFY no hardcoded date filters remain:
   - Search for "new Date()" in page.tsx - should only be in filter defaults, not in card calculations
   - Search for "today" in stat card logic - remove any hardcoded today date filters
   - All date filtering must come from searchParams, not hardcoded values

7. Remove Ads-related stat card display:
...
8. Keep recent orders and low stock sections (they use filtered/unfiltered data appropriately)
  </action>
  <verify>
Run: npm run build
...
CRITICAL VERIFICATION - No hardcoded date filters:
Run: grep -n "startOfDay\|endOfDay\|new Date()" src/app/(dashboard)/dashboard/page.tsx
Should NOT show any date calculations in card values - all dates come from searchParams.
  </verify>
  <done>Dashboard page uses getFilteredDashboardStats, all visible metrics respect date/store filters, no hardcoded date overrides</done>
</task>
