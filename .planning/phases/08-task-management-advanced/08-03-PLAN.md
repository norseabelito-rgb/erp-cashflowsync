...
must_haves:
  truths:
    - "AWB batch creation automatically creates a picking task"
  ...
  artifacts:
    - path: "src/app/api/orders/process-all/route.ts"
      provides: "Auto-task creation hooks for AWB batch and errors"
  ...
  key_links:
    ...
    - from: "src/app/api/orders/process-all/route.ts"
      to: "createAutoTask"
      via: "function call after AWB batch"
      pattern: "createAutoTask\\(\"AWB_BATCH_CREATED\""
---

<objective>
Hook auto-task creation into the order processing flow for AWB batches and processing errors.

Purpose: System events automatically create related tasks (TASK-07 requirement).
Output: process-all/route.ts calls createAutoTask after AWB batch creation and on processing errors.
</objective>
...
2. After AWB batch creation (after the pickingList is created and notifications are sent to pickers - around line 562):
...
```
  ...
  <verify>grep -A5 "linkedOrderId" src/lib/task-service.ts shows order linking is implemented</verify>
  <done>AUTO_TASK_CONFIG templates match available context, linkedOrderId is properly connected</done>
</task>
