...
must_haves:
  truths:
    - "AWB statuses are consistently categorized across all pages (dashboard, tracking, orders)"
    - "Unknown FanCourier status codes appear in admin settings page for manual mapping"
    - "All 52+ FanCourier status codes have defined categories and action recommendations"
    - "Status lookup by code returns the same result as string-based lookup for known statuses"
  artifacts:
