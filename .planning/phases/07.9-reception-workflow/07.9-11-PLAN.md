...
<task type="auto">
  <name>Task 1: Audit and identify low stock implementation locations</name>
  <files>src/lib/dashboard-stats.ts, src/app/api/dashboard/route.ts</files>
  <action>
Audit where low stock is currently calculated and identify locations needing migration:

1. Search for "stockQuantity" or "lowStock" in dashboard-related files:
   - grep for these patterns in src/lib/dashboard-stats.ts
   - grep in src/app/api/dashboard/route.ts
   - grep in src/app/(dashboard)/page.tsx

2. For each location found, document:
   - File path and line number
   - Current implementation (Product.stockQuantity)
   - What needs to change (InventoryItem.currentStock)

3. Check for existing functions:
   - getLowStockItems() or similar
   - getLowStockCount() or similar
...
**Output a brief summary of locations found and what needs changing.**
  </action>
  <verify>
    - List all files/functions using Product.stockQuantity for low stock alerts
    - Document the migration path for each location
  </verify>
  <done>Identified and documented all locations needing migration from Product to InventoryItem</done>
</task>
